        ...
        <section class="section">
            ...
            <div class="grid courses">
                @forelse($courses as $course)
                    <article class="card">
                        <span class="eyebrow">{{ $course->category }} · {{ $course->level }} · {{ $course->status === 'published' ? 'Aktif' : 'Nonaktif' }}</span>
                        <h3>{{ $course->title }}</h3>
                        <p>{{ $course->summary }}</p>
                        <div class="meta">
                            <span class="badge">{{ optional($course->mentor)->name ?? 'Mentor belum dipilih' }}</span>
                            <span class="badge">{{ $course->price > 0 ? 'Rp' . number_format($course->price, 0, ',', '.') : 'Gratis' }}</span>
